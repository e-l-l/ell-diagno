...
export const generateCase = async () => {
  try {
    const response = await axios.post(`${BaseURL}/cases/generate`);
...
export const getUnusedCases = async () => {
  try {
    const response = await axios.get(`${BaseURL}/cases/unused`);
    console.log("Unused cases response", response.data);
    return response.data;
  } catch (error: any) {
    if (error.code === "ERR_NETWORK") {
      console.error(
